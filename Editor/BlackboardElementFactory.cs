...
{ 
    ...
    public static EventSO CreateEvent(EventType type, string id = null)
    {
        EventSO eventSO = type switch
        {
            EventType.Actor => ScriptableObject.CreateInstance<EventActorSO>(),
            EventType.Item => ScriptableObject.CreateInstance<EventItemSO>(),
            _ => throw new ArgumentOutOfRangeException(nameof(type), type, null)
        };
